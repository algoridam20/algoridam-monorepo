...
tableHeaders = ['Name', 'Address', 'Date', 'Vaccine type',
                'Available capacity', 'Slots', 'Min age']
style1 = 'fancy_grid'
style2 = 'html'
...
def actionItem(case, url, data, emailList):
    if(case == 1):
        ...
        sendEmail(emailFrom, emailFromPassword, emailList, tabulate(
            data, headers=tableHeaders, tablefmt=style2))
    elif(case == 2):
...
def eventLoop():
    try:
        ...
        for location in locationMailList:
            getAvailableSlots(location[1], todaysDate,
                              minAgeLimit, location[0])
